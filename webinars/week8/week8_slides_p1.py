...
dt_now = dt.datetime.now() #print(dt_now)---->2024-04-02 18:31:56.379814
dt_now.year #2024
dt_now.month #4
repr(dt_now) #python will guess how dt_now is created, output--->datetime.datetime(2024, 4, 2, 18, 31, 56, 379814)
...
utils.pprint(dt1.minute, 'dt1.minute') #if didnt define minute,hour… it will show 0
...
start = dt.datetime(year=2000,month=11,day=5,hour=12,minute=30,second=20)
end = dt.datetime.now()
delta_alive = end - start
print(delta_alive)
print(repr(delta_alive))
alive_sec = (delta_alive.days * 24 * 60 * 60) + delta_alive.seconds
res = f"I have been alive for {alive_sec:,.0f} secs"
print(res)

# 2. How old will you be in 1,340 days
days = 1340
future = dt.datetime.now() + dt.timedelta(days=days)
future_alive = future - start
age = future_alive.days/365
